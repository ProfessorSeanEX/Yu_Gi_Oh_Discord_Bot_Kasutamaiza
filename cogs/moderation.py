...
- Provides tools to manage the server (e.g., kick, ban, mute, warn).
"""
...
class Moderation(commands.Cog):
    ...
    async def slash_kick(self, ctx: discord.ApplicationContext, member: discord.Member, reason: str = "No reason provided."):
        ...
        try:
            ...
            await ctx.respond(f"Member {member} has been kicked for: {reason}")
    ...
    async def slash_ban(self, ctx: discord.ApplicationContext, member: discord.Member, reason: str = "No reason provided."):
        ...
        try:
            ...
            await ctx.respond(f"Member {member} has been banned for: {reason}")
        except discord.Forbidden:
    ...
    async def slash_unmute(self, ctx: discord.ApplicationContext, member: discord.Member):
        ...
        try:
            ...
            if muted_role in member.roles:
                ...
                await ctx.respond(f"{member} is not currently muted.", ephemeral=True)
        except Exception as e:
    ...
    async def slash_warn(self, ctx: discord.ApplicationContext, member: discord.Member, reason: str = "No reason provided."):
        ...
        try:
            await ctx.respond(f"{member} has been warned: {reason}")
...
def setup(bot: discord.Bot, guild_id: int):
    ...
    bot.add_cog(Moderation(bot, guild_id))
    logger.info("Moderation cog has been added.")
